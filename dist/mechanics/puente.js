...
exports.hacerPuente = hacerPuente;
// Mecánica para hacer puentes sobre huecos profundos
function hacerPuente(bot) {
    const forward = bot.entity.position.offset(0, -1, 2);
    const blockBelow = bot.blockAt(forward);
    if (!blockBelow || blockBelow.name === 'air') {
        const bloques = bot.inventory.items().filter((i) => i.name.includes('planks') || i.name.includes('stone') || i.name.includes('dirt'));
        if (bloques.length > 0) {
